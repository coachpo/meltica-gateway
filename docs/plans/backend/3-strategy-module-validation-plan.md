...
- Modify the handler that persists modules (likely `internal/app/...` orchestrator invoked from `cmd/gateway`) to map `DiagnosticError` to HTTP 422 with payload:
  ```json
  {
    ...
    "diagnostics": [{ "stage": "compile", "message": "...", "line": 42, "column": 3 }]
  }
  ```
...
## Dependencies
- Coordination with frontend to consume diagnostics (see companion plan in `docs/plans/frontend/strategy-module-editor-plan.md`).
